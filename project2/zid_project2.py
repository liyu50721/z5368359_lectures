...
def read_prc_csv(tic):
    ...
    """
    #<COMPLETE THIS PART>
    # Construct the file path using the ticker and DATADIR from config.py
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    prc_df = pd.DataFrame(index=pd.to_datetime([]))

    # Iterate through tickers and read stock prices using read_prc_csv
    for ticker in tickers:
        # Read stock prices for the current ticker
        tic_df = read_prc_csv(ticker)

        # Rename the column of interest to prc_col
        tic_df.rename(columns={prc_col: ticker.lower()}, inplace=True)

        # Merge the data into the main DataFrame
        prc_df = pd.concat([prc_df, tic_df[ticker.lower()]], axis=1)
...
def get_avg(df, col, year):
    ...
    """
    #<COMPLETE THIS PART>
    col_data = df[df.index.year == year]
    avg = col_data[col].dropna().mean()
...
def get_ew_rets(df, tickers):
    ...
    """
    #<COMPLETE THIS PART>
    col_selected = df[tickers]
    # print(selected_columns)
    result = col_selected.mean(axis=1, skipna=True)
...
def get_ann_ret(ser, start, end):
    ...
    """
    # <COMPLETE THIS PART>
    dates = ser.loc[start:end]

    # tot_ret = (dates + 1).prod()

    # non_missing_returns = dates.count()

    # calculate result for annualized return
    result = ((dates + 1).prod() ** (252 / dates.count())) - 1
