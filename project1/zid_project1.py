...
COLUMNS = ['Volume', 'Date', 'Ajd Close', 'Close', 'Open', 'High']
...
#
COLWIDTHS = {'Volume': 14, 'Date': 11, 'Ajd Close': 19, 'Close': 10, 'Open': 6, 'High': 20}
...
def get_tics(pth):
    ...
    with open(pth, "r") as file:
        ...
        while line:
            ...
            res[line_parts[1].lower().strip('"\n\t ')] = line_parts[0].lower().strip('"\n\t ')
            line = file.readline()
    return res
...
def read_dat(tic):
    ...
    file_location = os.path.join(DATDIR, f"{tic}_prc.dat")
    content = []
...
def line_to_dict(line):
    ...
    start_index = 0
    for item in COLUMNS:
        end_index = start_index + COLWIDTHS[item]
        res[item] = line[start_index:end_index].strip()
        start_index = start_index + COLWIDTHS[item]
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is not None:
        ...
        for item in tickers_lst:
            if item not in tic_exchange_dic.keys():
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    if tickers_lst is not None:
        filtered_tic_exchange_dic = {key: tic_exchange_dic[key] for key in tickers_lst}
        res = data_assemble(filtered_tic_exchange_dic, col_lst)
    else:
        res = data_assemble(tic_exchange_dic, col_lst)

    return res


def data_assemble(dic, col_lst=None):
    res = {}
    for key, value in dic.items():
        item_info = {}
        item_info['exchange'] = value
        data_list = []
        data = read_dat(key)

        if col_lst is None:
            for i in range(0, len(data)):
                data_list.append(line_to_dict(data[i]))

        else:
            for i in range(0, len(data)):
                data_list.append({key: line_to_dict(data[i])[key] for key in col_lst})

        item_info['data'] = data_list
        res[key] = item_info
...
if __name__ == "__main__":
    ...
    # line_to_dict('00000000000000 2005-09-27018.19491004943847719.299999221.0490021.399999618530273')
    # _test_line_to_dict()

    # tic_exchange_dic = {'tsm': 'nyse', 'aal': 'nasdaq'}
    # # 'Tsm' is not a key in tic_exchange_dic
    # tickers_lst = ['aal', 'Tsm']
    #
    # try:
    #     verify_tickers(tic_exchange_dic, tickers_lst)
    ...
    # # 'close' is not found in COLUMNS
    # col_lst = ['Close', 'Volume', 'Open']
    #
    # try:
    #     verify_cols(col_lst)
    # except Exception as e:
    ...
    # create_data_dict({'aapl': 'nasdaq'}, None, ['Date', 'Close'])
